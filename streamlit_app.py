...
df['Risk Status'] = np.where(df['RUL (Days)'] < 60, 'CRITICAL',
np.where(df['RUL (Days)'] < 120, 'HIGH', 'LOW'))
return df.sort_values(by='RUL (Days)', ascending=True)
...
with col1:
st.metric(label="Total Cost Avoided YTD (Pilot)",
value=f"${total_cost_avoided:,.0f}",
delta="Target: $5,000,000")

with col2:
st.metric(label="Assets in CRITICAL Status (<60 Days RUL)",
value=critical_assets,
delta="1 Asset Moved to Low-Risk Last Week",
delta_color="normal")

with col3:
st.metric(label="Average Portfolio RUL",
value=f"{df_rul['RUL (Days)'].mean():.1f} Days")
